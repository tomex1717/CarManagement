...
function initMap() {
    ...
    map = new google.maps.Map(document.getElementById("map"), {
        ...
        center: {
            lat: jsonVar[(Object.keys(jsonVar).length) - 1].lat,
            lng: jsonVar[(Object.keys(jsonVar).length) - 1].lng
        },
        zoom: 16
    });
    ...
    var positionMarker = new google.maps.Marker({
        position: new google.maps.LatLng(
            (jsonVar[(Object.keys(jsonVar).length) - 1].lat), jsonVar[(Object.keys(jsonVar).length) - 1].lng),
        icon: bird_icon,
